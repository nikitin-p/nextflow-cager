process HISAT2_BUILD {
    ...
    if (avail_mem >= hisat2_build_memory) {
        extract_exons = "$gtf" != 'NO_FILE_GTF' ? "hisat2_extract_exons.py $gtf > ${gtf.baseName}.exons.txt" : ""
        ss = "$splicesites" != 'NO_FILE_SPLICESITES' ? "--ss $splicesites" : ""
        exon = "$gtf" != 'NO_FILE_GTF' ? "--exon ${gtf.baseName}.exons.txt" : ""
    } else {
